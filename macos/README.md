...
> Library and test targets are the primary verification surface in Linux CI. The SwiftUI app target must be built on macOS.
...
The Windows `.NET` / WPF tree at the repository root is retained as the behavioral reference. Do not delete it until macOS feature parity is reached.
